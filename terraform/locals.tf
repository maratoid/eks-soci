locals {
  ...
  containerd_config_template = <<-EOT
    {{- /* */ -}}
    ...
    {{ if eq .NodeConfig.AgentConfig.Snapshotter "soci" }}
    [proxy_plugins]
      [proxy_plugins.soci]
        type = "snapshot"
  ...
  master_cloud_init = <<-EOT
    ...
    write_files:
      ...
      - path: /var/lib/rancher/k3s/agent/etc/containerd/config-v3.toml.tmpl
        permissions: '0644'
        content: |
          ${indent(6, local.containerd_config_template)}
    runcmd:
      - /usr/local/bin/soci-setup.sh
      - curl -sfL https://get.k3s.io | K3S_TOKEN=${local.k3s_token} sh -s - server --cluster-init --node-label "role=master" --snapshotter=soci
  ...
  worker_cloud_init = <<-EOT
    ...
    runcmd:
      - /usr/local/bin/soci-setup.sh
      - curl -sfL https://get.k3s.io | K3S_URL=https://${multipass_instance.master_node.ipv4[0]}:6443 K3S_TOKEN=${local.k3s_token} sh -s - agent --node-label "role=worker" --snapshotter=soci
  EOT
}
